...
export function SyncConnectDialog({ open, onClose }: Props) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      try {
        localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify({ ...form, nick }));
      } catch {
        // ignore
      }
      onClose();
      ...
      >
        ...
        <div className="modal-body">
          <p className="muted small">
            kj-trace-server に WebSocket で接続します．ルームが招待制の場合はメアドが必要．
            学外から接続する場合は事前に大学 VPN に入ってください．
          </p>
          ...
          <div className="form-row">
            ...
            <span className="muted small">
              招待制ルームでは必須．自分が招待リストに登録されたメアドを入力．
            </span>
          </div>
